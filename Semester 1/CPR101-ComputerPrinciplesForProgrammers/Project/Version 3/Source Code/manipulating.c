...
take two string inputs from the user until he enters 'q' to quit the input cycle.
	...
	*/
	...
	// Declare variables for input and substring search
	...
	// Start a loop that continues until the user enters "q"
		...
		// Check if the user input is not "q"
			 ...
			 {
				// Prompt the user to input a substring
				printf("Type the substring: \n");
				fgets(needle, BUFFER_SIZE, stdin);
				// Remove the newline character from the substring input
				needle[strlen(needle) - 1] = '\0';
				// Search for the occurrence of the substring within the input string
				occurrence = strstr(haystack, needle);
				// Check if the substring was found
				if (occurrence)
					// Print the substring and its position within the input string
					printf("\'%s\' found at %d position\n", needle,(int)(occurrence - haystack));
				else
					// Print "Not found" if the substring was not found
					printf("Not found\n");
			 }
		} while (strcmp(haystack,"q") != 0);  // Continue looping until the user enters "q"
		// Print an end message for the demo'+ check output with no input
